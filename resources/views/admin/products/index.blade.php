		...
		<tbody>
			...
			{{-- 		{{ __($product->price) }} VND --}}
				...
				<form action="{{ route('admin.products.delete', [$product->id]) }}" method="POST" role="form">							@method('DELETE')
 					 @csrf
 					 <button type="submit" class="btn btn-danger" name="delete" value="{{ $product->id }}" onclick="return confirm('Bạn có chắc muốn xóa sản phẩm')"><i class="fad fa-trash">Xoá</i></button>
				</form>
